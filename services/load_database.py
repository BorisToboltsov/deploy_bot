...
async def load_database(db_name: str, telegram_id: int, state: FSMContext) -> NoReturn:
    ...
    subprocess.run(["mysql", "-u", os.getenv("USER_BACKUP"), "-p", os.getenv("PASSWORD_BACKUP"), "-Bse", '"RESET MASTER;"'], stdout=subprocess.PIPE)
    ...
    # os.system(load)
    subprocess.run(["mysql", "-u", os.getenv("USER_BACKUP"), "-p", os.getenv("PASSWORD_BACKUP"), db_name, "<", f"{os.getenv("PATH_TO_BACKUP")}/{db_name}_{current_date}.sql"], stdout=subprocess.PIPE)
